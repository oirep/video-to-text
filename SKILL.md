...
name: video-to-text
version: 1.0.0
description: "将本地会议视频文件（MP4/M4A/MOV）转换为带时间戳的 Markdown 文字稿，同时分析屏幕共享内容（PPT、代码、白板、表格）。语音识别使用本地 faster-whisper（中文 medium 模型，无需联网），屏幕内容使用 Claude Vision 分析，输出语音逐字稿与屏幕画面交织的完整时间线。适用于：飞书妙记 API 失败、腾讯会议录制（无飞书 API）、含屏幕共享需要视觉内容的录制。当用户说「把这个视频转文字」「录制转文本」「分析这个MP4」「把这个会议录制转成文字」「视频转文字稿」时触发。"
metadata:
  requires:
    bins: ["ffmpeg", "python3"]
